...
class spawn_windows_conpty(object):
    ...
    Provides a pexpect-like interface.
    """
    def __init__(
        self,
        command,
        ...
        self._pty_in = HANDLE(INVALID_HANDLE_VALUE)
        self._pty_out = HANDLE(INVALID_HANDLE_VALUE)
        self._child_stdin = HANDLE(INVALID_HANDLE_VALUE)
        self._child_stdout = HANDLE(INVALID_HANDLE_VALUE)
        self._proc_info.hProcess = HANDLE(INVALID_HANDLE_VALUE)
        self._proc_info.hThread = HANDLE(INVALID_HANDLE_VALUE)

        self._reader_thread = None; self._read_queue = queue.Queue()
        self._process_ended_event = threading.Event(); self._monitor_thread = None
    ...
    def _spawn(self):
        ...
        if self.env is not None:
            ...
            env_block = env_strings.encode('utf-16le')
    ...
    def _read_loop(self):
        ...
        finally: self._read_queue.put(None); # log("ConPTY: Reader thread finished.") # Optional
